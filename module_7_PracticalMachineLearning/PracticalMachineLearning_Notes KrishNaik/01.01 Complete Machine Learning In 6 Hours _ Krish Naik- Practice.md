...
- The Cost Function is used to find out the best fit line such that there is minimal distance between the data points and predicted data points, and cost function is implementd using the below equation which is the cost function equation
  ...
  - ![Cost-Function-{J(\theta_0, \theta_1) = \frac{1}{2m} \sum_{i=1}^{m} (h_{\theta}(x^{(i)}) - y^{(i)})^2} (https://latex.codecogs.com/svg.image?{J(\theta_0,\theta_1)=\frac{1}{2m}\sum_{i=1}^{m}(h_{\theta}(x^{(i)})-y^{(i)})^2})](../../content/Cost-Function-{J(_theta_0,_theta_1)=_frac{1}{2m}_sum_{i=1}^{m}(h_{_theta}(x^{(i)})%20-%20y^{(i)})^2}.svg)

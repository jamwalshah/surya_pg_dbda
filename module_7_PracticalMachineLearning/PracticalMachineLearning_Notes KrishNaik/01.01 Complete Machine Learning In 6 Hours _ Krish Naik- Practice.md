...
- This equation of line/linear regression is also represented by
  - y     = β₀ + β₁x
  - ![h_{\theta}(x) = \theta_0 + \theta_1x (https://arachnoid.com/latex/?equ=h_%7B%5Ctheta%7D(x)%20%3D%20%5Ctheta_0%20%2B%20%5Ctheta_1x)](../../content/h_{_theta}(x)=_theta_0+theta_1x.gif)
    - it is Linear Hypothesis or linear regression model equation where
      - 'y' or ![Predicted-DataPoint-{h_{\theta}(x)} (https://arachnoid.com/latex/?equ=h_%7B%5Ctheta%7D(x))](../../content/Predicted-DataPoint-{h_{_theta}(x)}.gif)
...
- As per Andrew NG, the equation of a straight line is
  - y = mx⁽ⁱ⁾ + c
  - y = β₀ + β₁x⁽ⁱ⁾
  - ![h_{\theta}(x^{(i)}) = \theta_0 + \theta_1x^{(i)} (https://arachnoid.com/latex/?equ=h_%7B%5Ctheta%7D(x%5E%7B(i)%7D)%20%3D%20%5Ctheta_0%20%2B%20%5Ctheta_1x%5E%7B(i)%7D)](../../content/h_{_theta}(x^{(i)})=_theta_0+_theta_1x^{(i)}.gif)
- To understand what is 'θ₀' and 'θ₁', lets say we have a problem statement, where we have some data points and we're trying to create a best fit line
  - the best fit line is given by the equation ![h_{\theta}(x) = \theta_0 + \theta_1x (https://arachnoid.com/latex/?equ=h_%7B%5Ctheta%7D(x)%20%3D%20%5Ctheta_0%20%2B%20%5Ctheta_1x)](../../content/h_{_theta}(x)=_theta_0+theta_1x.gif)
  
  - ![Equation-of-a-Straight-Line](../../content/Equation-of-a-Straight-Line.png)
  
  - 'θ₀' is the intercept, which means when x = 0, then ![Predicted-DataPoint-{h_{\theta}(x)} (https://arachnoid.com/latex/?equ=h_%7B%5Ctheta%7D(x))](../../content/Predicted-DataPoint-{h_{_theta}(x)}.gif) = θ₀
    - intercept basically indicates that at what point the best fit line meets the y-axis
...
- The hypothesis is ![{h_{\theta}(x) = \theta_0 + \theta_1x} (https://arachnoid.com/latex/?equ=h_%7B%5Ctheta%7D(x)%20%3D%20%5Ctheta_0%20%2B%20%5Ctheta_1x)](../../content/h_{_theta}(x)=_theta_0+theta_1x.gif)
  - 'θ₀' and 'θ₁' are the weights that needs to be updated
- The Cost Function is used to find out the best fit line such that there is minimal distance between the data points and predicted data points, and cost function is implementd using the below equation which is the cost function equation
  - ![Cost-Function {J(\theta_0, \theta_1) = \frac{1}{2m} \sum_{i=1}^{m} (h_{\theta}(x^{(i)}) - y^{(i)})^2} (https://arachnoid.com/latex/?equ=J(%5Ctheta_0%2C%20%5Ctheta_1)%20%3D%20%5Cfrac%7B1%7D%7B2m%7D%20%5Csum_%7Bi%3D1%7D%5E%7Bm%7D%20(h_%7B%5Ctheta%7D(x%5E%7B(i)%7D)%20-%20y%5E%7B(i)%7D)%5E2)](../../content/Cost-Function-{J(_theta_0,_theta_1)=_frac{1}{2m}_sum_{i=1}^{m}(h_{_theta}(x^{(i)})%20-%20y^{(i)})^2}.png) , where
    - ![Predicted-DataPoints-{h_{\theta}(x^{(i)})} (https://arachnoid.com/latex/?equ=%7Bh_%7B%5Ctheta%7D(x%5E%7B(i)%7D)%7D)](../../content/Predicted-DataPoints-{h_{_theta}(x^{(i)})}.png)
    ...
    - ![Squared-Distance-Predicted-Actual-{(h_{\theta}(x^{(i)}) - y^{(i)})^2} (https://arachnoid.com/latex/?equ=%7B(h_%7B%5Ctheta%7D(x%5E%7B(i)%7D)%20-%20y%5E%7B(i)%7D)%5E2%7D)](../../content/Squared-Distance-Predicted-Actual-{(h_{_theta}(x^{(i)})-y^{(i)})^2}.png)
      - represents the square of the distance between the predicted data points and the actual data points
    - ![Sum-Squared-Distances-Predicted-Actual-{\sum_{i=1}^{m} (h_{\theta}(x^{(i)}) - y^{(i)})^2} (https://arachnoid.com/latex/?equ=%7B%5Csum_%7Bi%3D1%7D%5E%7Bm%7D%20(h_%7B%5Ctheta%7D(x%5E%7B(i)%7D)%20-%20y%5E%7B(i)%7D)%5E2%7D)](../../content/Sum-Squared-Distances-Predicted-Actual-{_sum_{i=1}^{m}(h_{_theta}(x^{(i)})-y^{(i)})^2}.png)
    ...
    - 1/2
      - used to simplify the equation for the derivation purpose
  - ![Cost-Function {J(\theta_0, \theta_1) = \frac{1}{2m} \sum_{i=1}^{m} (h_{\theta}(x^{(i)}) - y^{(i)})^2} (https://arachnoid.com/latex/?equ=J(%5Ctheta_0%2C%20%5Ctheta_1)%20%3D%20%5Cfrac%7B1%7D%7B2m%7D%20%5Csum_%7Bi%3D1%7D%5E%7Bm%7D%20(h_%7B%5Ctheta%7D(x%5E%7B(i)%7D)%20-%20y%5E%7B(i)%7D)%5E2)](../../content/Cost-Function-{J(_theta_0,_theta_1)=_frac{1}{2m}_sum_{i=1}^{m}(h_{_theta}(x^{(i)})%20-%20y^{(i)})^2}.png)

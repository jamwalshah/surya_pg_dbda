...
- Suppose I have two feaatures
    1. X : Age
    2. Y : Weight, then is it necessary hat we should use both the features in such case
    aa
...
2. **Standardization**
    ...
    - Wherever you have gradient descent involved, it is good to do standardization, because if our initial point is a point which is scaled, it is quick to reach the global minima or training the model, otherwise initial point whill be much away from global mimina, so it'll take more time to reach global minima
...
4. **Multi-Collinearity**
    - Multi-collinearaity means how much correlated two features are
    - Example
    ...
    - Another term `Homoscedasticity`, which is the homogenity of variances, is also used to solve multi-collinearity
...
- Apart from preventing overfitting, it also helps in feature selection
  ...
  - Comparing with Ridge Regression which uses square of slope (${slope^2}$ or ${(θ_1)^2}$), so value inceases by a huge difference
  - In Lasso regession, best fit line is represented by\
  ${h_θ(x) = ŷ = θ_0 + θ_1x_1 + θ_2x_2 + θ_3x_3 + ... + θ_nx_n}$, \
  so absolute value of slope is\
  ${|slope| = |θ_0 + θ_1 + θ_2 + θ_3 + ... + θ_n|}$
  - Now the slope is represented as its true value in absolute form, and if a feature has insignificant slope, then such feature can be neglected as its value is not increasing unlike Ridge regession
- Lasso Regression is important because it helps in
  1. Preventing overfitting
